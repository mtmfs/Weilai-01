...
const w = s => {
  ...
  if (fileSink) { try { appendFileSync(fileSink, `${stamp()} ${stripAnsi(s)}\n`, 'utf8'); } catch (e) {} } // 文件写失败不影响控制台/不抛
};

// 开启文件日志镜像：后续所有 stderr 日志（去色 + 本地时间戳）追加到 path。建文件失败则退回仅控制台（warn 一次、不抛）。
// 控制台输出不变（保留颜色、不加时间戳）；stdout 的 out() 不进文件（机器 JSON 与运行日志分离）。
export function enableFileLog(path) {
  try { mkdirSync(dirname(path), { recursive: true }); appendFileSync(path, '', 'utf8'); fileSink = path; return path; }
  catch (e) { process.stderr.write(`⚠ 无法开启文件日志 ${path}：${e.message}（仅控制台）\n`); fileSink = null; return null; }
}
