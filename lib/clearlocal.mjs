...
// 孤儿副本：md5fix 输出目录里"源已不在 flatRoot"的残留（旧批次/源已删），纯可再生垃圾（整合原 gc 设想）。
function findOrphans(md5Base, channelKeys, localSet, kw) {
  const dirs = [md5Base, ...channelKeys.map((ch) => join(md5Base, ch))];
  const orphans = [];
  for (const dir of dirs) {
    let entries = [];
    try { entries = readdirSync(dir); } catch (e) { continue; }
    for (const n of entries) {
      if (!/\.(mp4|mov|m4v|avi)$/i.test(n)) continue;
      if (kw && !n.includes(kw)) continue;  // kw 闸门
      if (localSet.has(n)) continue;         // 源还在 → 不是孤儿
      try { if (statSync(join(dir, n)).isFile()) orphans.push(join(dir, n)); } catch (e) {}
    }
  }
  return orphans;
}
...
export function runClearLocal(system, { apply = false, log } = {}) {
  ...
  const orphans = findOrphans(md5Base, Object.keys(state.channels || {}), localSet, kw); // ★整合原 gc
  ...
  L.info(`清洗计划：delivered→★彻底删除 ${cnt.delivered} ｜ scrapped→内容不合格/ ${cnt.scrapped} ｜ md5fix 孤儿副本→删 ${orphans.length} ｜ flatRoot 视频 ${localFiles.length}（${kw || '全部受管'} ${kwFiles.length}）`);
  plan.forEach(p => L.info(`  [${p.stage}] ${p.file}  →  ${p.action === 'delete' ? '★彻底删除' : p.dest + '/'}`));
  orphans.forEach(o => L.info(`  [孤儿] ${o}  →  删（源已不在 flatRoot）`));

  if (!apply) { L.warn(`[dry-run] 未动盘。真清洗加 --apply（删 ${cnt.delivered} 已交付源 + 移 ${cnt.scrapped} 作废 + 删 md5fix 衍生 + 删 ${orphans.length} 孤儿副本）`); return { dryRun: true, plan: plan.length, orphans: orphans.length, scrapped: cnt.scrapped, delivered: cnt.delivered }; }
  if (!plan.length && !orphans.length) { L.ok('无终态件 / 无孤儿可清洗'); return { deleted: 0, moved: 0, derived: 0, orphans: 0, plan: 0, scrapped: 0, delivered: 0 }; }
  ...
  for (const p of plan) {
    ...
    for (const d of [...Object.keys(state.channels).map(ch => join(md5Base, ch, p.file)), join(md5Base, p.file)]) {
      try { if (existsSync(d)) { rmSync(d, { force: true }); derived++; } } catch (e) {}
    }
  }
  // ★孤儿副本清理（整合原 gc）：源已删的 md5fix 残留。
  let orphanDel = 0;
  for (const o of orphans) { try { if (existsSync(o)) { rmSync(o, { force: true }); orphanDel++; } } catch (e) {} }

  L.ok(`clear-local: 彻底删除 ${deleted}（已交付）｜移 ${moved} → 内容不合格/（作废）｜删 md5fix 衍生 ${derived}｜删孤儿 ${orphanDel}`);
  return { deleted, moved, derived, orphans: orphanDel, plan: plan.length, scrapped: cnt.scrapped, delivered: cnt.delivered };
}
