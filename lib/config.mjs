...
// Windows Chrome 标准安装位（换机时 system.json 的 chrome.path 陈旧、但 Chrome 装在标准位 → 运行时兜底/doctor --fix 修补）。
export const CHROME_CANDIDATES = [
  'C:\\Program Files\\Google\\Chrome\\Application\\chrome.exe',
  'C:\\Program Files (x86)\\Google\\Chrome\\Application\\chrome.exe',
  ...(process.env.LOCALAPPDATA ? [process.env.LOCALAPPDATA + '\\Google\\Chrome\\Application\\chrome.exe'] : []),
];
// 取候选里首个存在的 chrome.exe（纯 existsSync 探测，不启动进程）。loadSystem 兜底 + sysrepair/doctor --fix 共用。
export function pickChrome(cands = CHROME_CANDIDATES) {
...
export function loadSystem() {
  ...
  // ★md5fix 纳入必填：md5fix.outDir 被 flywheel/md5fix/upload/clearlocal/doctor 直接消费，缺它会在使用点 TypeError 崩；
  //   提前到此早抛清晰 E_CONFIG（坏档可 `doctor --fix` 修复——它读原始 JSON、不过本函数，链路自洽）。
  for (const k of ['project', 'chrome', 'ffmpeg', 'md5fix', 'timeouts', 'concurrency']) {
    if (!sys[k]) throw cfgErr(`system.json 缺字段：${k}`);
  }
  if (!sys.project.ledgerPath) throw cfgErr('system.json.project 缺 ledgerPath');
  if (!sys.md5fix.outDir) throw cfgErr('system.json.md5fix 缺 outDir');
  sys.chrome.viewport = parseWindowSize(sys.chrome.flags); // ★视口单一真源（喂 hardenViewport，见 cdp.mjs）
  // chrome.path 运行时兜底：配置路径不存在但 Chrome 装在标准位 → 解析到现存可执行（仅内存、不改文件；持久化交 doctor --fix）。
  if (!sys.chrome.path || !existsSync(sys.chrome.path)) { const alt = pickChrome(); if (alt) sys.chrome.path = alt; }
  return sys;
}
...
// 命令层标签解析：free/paid/真实 id → 真实 id；未知值报 E_USAGE，而不是落到 loadTarget 的 E_CONFIG。
// 用于 status/config 这类 raw 参数命令，它们自己解析位置参数，不经 dispatcher 的通道解析。
export function resolveChannelArg(label, usageHint) {
  const id = labelToId(label);
  const ids = discoverChannelIds();
  if (!ids.includes(id)) {
    const e = new Error(`${usageHint}（通道: free|paid|${ids.join('|')}）`);
    e.code = 'E_USAGE';
    throw e;
  }
  return id;
...
// free/paid 命令层标签 → 内部通道 id（台账键仍是 jie3/jie6，标签只在命令面）。
// 其余（both/system/原始 id 如 jie3/jie6）透传。
export function labelToId(label) {
  if (label === 'free') return channelRegistry().testId;
  if (label === 'paid') return channelRegistry().delivId;
  return label;
}
