...
export async function probeAccount(/* cdp */) { return '?'; }
export async function waitAccount(/* cdp, timeoutMs = 15000 */) { return '?'; }
...
export async function probePlan(cdp, ui, planId) {
  ...
  return cdp.ev(`location.href.includes('adId=${planId}')?1:0`);
}
...
export async function probeSnapshot(cdp, { planId, ui } = {}) {
  const planExpr = ui === 'drawer'
    ...
    : `(location.href.includes('adId=${planId}')?1:0)`;
  try {
