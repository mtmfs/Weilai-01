...
// ★S8: 一次往返拿 account/url/view/plan，替代热路径多次顺序探针往返。
export async function probeSnapshot(cdp, { planId, ui } = {}) {
  const planExpr = ui === 'drawer'
    ...
    : `((location.href.includes('adId=${planId}')&&document.documentElement.outerHTML.includes('${planId}'))?1:0)`;
  try {
    const s = await cdp.j(`return JSON.stringify({acc:${ACC_RE},url:location.href,view:__byText(/^添加视频$/).length>0,plan:${planExpr}});`);
    return JSON.parse(s);
  } catch (e) { return { acc: '?', url: '', view: false, plan: 0 }; }
}
...
export async function ready(cfg, { secrets, log, attempts = 3 } = {}) {
  ...
  if (tab) {
    ...
    try {
      ...
      await hardenViewport(cdp); // ★视口硬化：lockPlan 等按坐标点击，钉死视口防点屏外（详见 cdp.mjs）
      await closePopup(cdp);
      const s = await probeSnapshot(cdp, { planId, ui }); // ★S8: 一次拿 acc/url/view/plan
      const accOk = s.acc === account && (s.url || '').includes('aavid=' + aavid);
      if (accOk && s.view) {
        L.ok(`已就绪（${account} / 计划${planId}），空转`);
        return { ready: true, skipped: true, account: s.acc, steps };
      }
      // ★C5: 账户/计划在位、仅视图(抽屉)关 → 只 open-view，不全冷重握手
      if (accOk && s.plan === 1 && !s.view) {
        L.step('账户/计划在位、仅视图关 → 只 open-view');
        await openView(cdp, ui, planId); await closePopup(cdp);
        if (await probeView(cdp)) {
          steps.push('open-view-only');
          L.ok(`已就绪（${account} / 计划${planId}），仅开视图`);
          return { ready: true, skipped: true, account: s.acc, steps };
        }
      }
  ...
  // 母账号登录核验（一次；retry 不重做）
  {
    ...
    try {
      ...
      await dcdp.send('Page.navigate', { url: 'https://agent.oceanengine.com' });
  ...
  for (let attempt = 1; attempt <= attempts; attempt++) {
    try {
      ...
      try {
        ...
        await hardenViewport(cdp); // ★视口硬化：冷收敛路径，lockPlan(clickAt) 前钉死视口（详见 cdp.mjs）
        await closePopup(cdp);
        const cur = await probeUrl(cdp);
        if (ui === 'drawer' && (!/uni-prom/.test(cur) || !cur.includes('aavid=' + aavid))) {
          await cdp.send('Page.navigate', { url: `https://qianchuan.jinritemai.com/uni-prom?aavid=${aavid}` });
        ...
        if (acc === '?') {
          ...
          L.step('账户未就绪 → 重锚再等');
          await cdp.send('Page.navigate', { url: reUrl });
          await waitFor(cdp, "document.readyState==='complete'", { timeoutMs: 8000, floorMs: 1500 }); await closePopup(cdp); // ★C1
