...
const ACC_RE = `(document.body.innerText.match(/聚量TS-(捷沅.)/)||[])[1]||'?'`;
...
export async function ready(cfg, { secrets, log } = {}) {
  const { system, target } = cfg;
  const profile = cfg.profile;
  const { port, aavid, ui, planId, account, advId } = { ...target, advId: target.advId };
  const steps = [];
  const L = log || { step() {}, ok() {}, warn() {} };

  // [探针] chrome-port → [动作] launch
  ...
  if (tab) {
    ...
    try {
      ...
      const u = await probeUrl(cdp);
      // 视图已开 + 账户对 + aavid 在 URL → 真空转（开了视图时抽屉盖住计划行，不再查行数）。
  ...
  {
    const dcdp = await connect({ port, urlSub: driver.url ? '' : 'uni-prom' });
    try {
      ...
      // 握手种千川会话
      L.step(`握手种会话 advId=${advId}`);
      await ssoHandshake(dcdp, advId); steps.push('handshake');
    } finally { dcdp.close(); }
  }
  ...
  steps.push('ensure-tab');

  // 在 aavid 标签上：重锚 → 账户断言 → 模式 → 计划 → 开视图
  const cdp = await connect({ port, aavid });
  try {
    ...
    if (ui === 'drawer' && (!/uni-prom/.test(cur) || !cur.includes('aavid=' + aavid))) {
      await cdp.send('Page.navigate', { url: `https://qianchuan.jinritemai.com/uni-prom?aavid=${aavid}` }); await sleep(10000);
    }
    if (!await probeSessionWarm(cdp)) throw Object.assign(new Error('会话冷/被弹回 login'), { code: 'E_SIG' });
    let acc = await waitAccount(cdp, 15000);
    if (acc === '?') { // 页面没加载好/落开通页 → 重锚一次再等
    ...
    L.ok(`账户=${account} ✓`);
    await setMode(cdp, target.mode, ui); steps.push('set-mode');
    if (!await lockPlan(cdp, ui, aavid, planId)) throw Object.assign(new Error(`锁计划 ${planId} 失败（模式错/计划不唯一）`), { code: 'E_SELECTOR' });
    steps.push('lock-plan');
    if (!await probeView(cdp)) { await openView(cdp, ui, planId); steps.push('open-view'); }
    await closePopup(cdp);
    const viewOk = await probeView(cdp);
    if (!viewOk) throw Object.assign(new Error('开视图后「添加视频」仍不可见'), { code: 'E_SELECTOR' });
    L.ok(`就绪（${account} / 计划${planId} / 视图开）`);
    return { ready: true, skipped: false, account: acc, steps };
  } finally { cdp.close(); }
}
