...
export async function assertIdentity(cdp, target) {
  ...
  // ★页面 reload 瞬间 probeAccount 会读成 '?'(账户元素未渲染)→误判 E_DRIFT、白触发 recoverOnce。
  //   仅在"读不出('?'或空)"时短重试几次给页面渲染时间；真·错账户(读出别的名)不重试、立即判漂移。
  let acc = await probeAccount(cdp);
  for (let i = 0; i < 6 && (!acc || acc === '?'); i++) { await new Promise(r => setTimeout(r, 800)); acc = await probeAccount(cdp); }
