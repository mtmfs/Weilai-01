...
// 每个碰浏览器的动作跑在 guard 里：动作前后 扫弹窗 + 断言身份（aavid/计划/会话），漂移/掉登录/签名过期自动恢复，恢复不了给稳定退出码。
...
export async function assertIdentity(cdp, target) {
  ...
  if (!(u || '').replace(/&amp;/g, '&').includes('aavid=' + target.aavid)) throw err('E_DRIFT', `aavid 不在位：${target.aavid}`);
