...
import { loadState, saveState, recomputeAll } from './state.mjs';
...
export async function runReconcile(cfg, { apply = false, platform = null, graceMin, log } = {}) {
  const L = log || { step() {}, ok() {}, info() {}, warn() {} };
  const { system, target } = cfg;
  const channel = target.id;
  const { kw, ledgerPath } = system.project;
  const gm = Number(graceMin); const graceMs = (Number.isFinite(gm) && gm >= 0 ? gm : DEFAULT_GRACE_MIN) * 60000;

  // 拉平台快照（无穿入则自连：入口护栏断言账户/计划/会话，绝不在错账户上对账；复用 sync 的签名捕获 + 翻页拉取）。
  let cdp = null;
  try {
    if (!platform) {
  ...
  const now = Date.now();
  const cands = computeReconcile(state, platform, channel, kw, graceMs, now);
  ...
  for (const c of cands) unbump(state.videos[c.key].ch[channel]);
  recomputeAll(state);
