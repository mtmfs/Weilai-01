...
export const DEFAULT_CHANNELS = {
  ...
  jie6: { role: 'delivery', account: '捷沅6', aavid: '1862076853297476', mode: '推直播间', planId: '1864536448309275', funded: true,  maxUploads: 2 }
};
...
export function loadState(p) {
  ...
  try {
    ...
    if (!s.channels) s.channels = base.channels;
    if (!s.pipeline) s.pipeline = base.pipeline;
...
export function observe(v, channelId, { audit, mid, ts }) {
  const c = v.ch[channelId]; if (!c) return;
  c.last_status = audit; if (mid) c.last_mid = mid; if (ts) c.last_ts = ts;
...
export function worklists(state) {
  ...
  for (const [k, v] of Object.entries(state.videos)) {
    const T = v.ch[testId], D = v.ch[delivId];
    if (v.stage === 'testing') { (T.uploads === 0 ? w.test_toupload : w.test_reupload).push(v.name); }
