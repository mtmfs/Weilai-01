...
export function loadState(p) {
  ...
  try { raw = readFileSync(p, 'utf8'); }
    ...
    // ★Windows 罕见：免锁 read 撞另一进程 rename 替换 p 的瞬间偶遇 EPERM/EBUSY（就地替换不产生 ENOENT）→ 极短重试一次；仍失败抛。
    // 半截内容不是 IO 错（读成功、JSON.parse 才失败）→ 由下方 .bak 兜底，不在此处理。
    if (e.code === 'EPERM' || e.code === 'EACCES' || e.code === 'EBUSY') {
      syncSleep(30);
      try { raw = readFileSync(p, 'utf8'); } catch (e2) { throw Object.assign(new Error(`读取台账失败 ${p}: ${e2.message}`), { code: 'E_CONFIG' }); }
    } else throw Object.assign(new Error(`读取台账失败 ${p}: ${e.message}`), { code: 'E_CONFIG' });
  }
  try {
...
// 同步无忙等睡眠（saveState 跑在 ledger 同步临界区内，不能用 setTimeout）：Atomics.wait 到 timeout。
...
export function saveState(p, s) {
  const tmp = p + '.tmp.' + process.pid; // ★进程唯一 tmp 名：多进程各写各的临时文件，rename 各自的，绝不互踩（防跨进程 tmp ENOENT）
  writeFileSync(tmp, JSON.stringify(s, null, 1), 'utf8');
  if (existsSync(p)) { try { copyFileSync(p, p + '.bak'); } catch (e) {} }
  // ★Windows：renameSync 遇并发免锁读者/杀软瞬时打开 p → EPERM/EACCES/EBUSY（MoveFileExW 句柄竞争）→ 同步短重试（≤10 次，退避）。
  for (let i = 0; ; i++) {
    try { renameSync(tmp, p); break; }
    catch (e) {
      if ((e.code === 'EPERM' || e.code === 'EACCES' || e.code === 'EBUSY') && i < 20) { syncSleep(Math.min(20 * (i + 1), 200)); continue; }
      throw e;
    }
  }
}
