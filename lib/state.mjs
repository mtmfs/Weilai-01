...
// ★通道事实唯一真源 = channels/*.json（loadState 经 bootChannels 从中取）。以下仅为"channels 读不到"时的
//   离线兜底（换机引导 / 不带 channels 的单测）；务必与 channels/*.json 保持一致（如 maxUploads）。
export const DEFAULT_CHANNELS = {
  jie3: { role: 'test',     account: '捷沅3', aavid: '1849209213181706', mode: '推商品',   planId: '1868230520126939', funded: false, maxUploads: 7 },
...
function bootChannels() {
  ...
  } catch (e) { /* 缺/坏 channels（离线/单测）→ 回退离线默认 */ }
  return { channels: JSON.parse(JSON.stringify(DEFAULT_CHANNELS)), pipeline: [...DEFAULT_PIPELINE] };
}
