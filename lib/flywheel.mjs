...
// 本通道待传/重传名单：新件(toupload)优先在前，返工(reupload)在后。
function channelWork(id, w) {
  return id === 'jie6'
  ...
  while (!signal.aborted) {
    ...
    try {
      ...
      if (due('fullsync', daemon.fullSyncMin)) {
        ...
        navigated = true;
        if (id === 'jie3' && due('delete', daemon.deleteMin)) {
          // jie6 delete 尚未实现（抛 E_CONFIG）→ 仅 jie3 清理；无 WIP 上限下 delete 只是清平台垃圾，非必需。
      ...
      const { toupload, reupload } = channelWork(id, w);
