...
export async function submit(cdp, cfg, { perFileNoProgressSec, patientMaxMin, pollSec, log } = {}) {
  ...
  const t0 = Date.now();
  let prevInprogSum = -1, stallMs = 0, s = null;
  while (Date.now() - t0 < maxMs) {
    await cdp.send('Page.bringToFront');
    s = await snap(cdp);
    L.info(`  传输 done=${s.done} inprog=${s.inprog} inprogSum=${s.inprogSum} 确定=${s.cEn}`);
    if (s.inprog === 0 && s.cEn) { L.ok('全部传完'); break; } // 最佳：全完成
    // ★逐文件超时：有完成件 + 确定可点 + 龟速件 % 停滞 noProg → 提交已完成的、踢回未完成的（不陪等整批）
    if (s.done > 0 && s.cEn) {
      if (s.inprogSum === prevInprogSum) stallMs += poll; else stallMs = 0;
      if (stallMs >= noProg) { L.warn(`龟速件 ${Math.round(noProg / 1000)}s 无进度 → 提交已完成 ${s.done} 个、踢回 ${s.inprog} 个`); break; }
    } else stallMs = 0;
