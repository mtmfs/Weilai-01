...
// statsFromFile: 读 JSONL → 分时段(hour-of-day) 请求数/各端点/错误率/req→resp 时长。
...
import { URL_RE } from './selectors.mjs';
import { createRotatingLineWriter, DEFAULT_TELEMETRY_ROTATION, forEachJsonlObject } from './artifacts.mjs';

// 只记 API/文档类，跳过静态资源
const interesting = (url, type) =>
  (URL_RE.telemetryBiz.test(url) && !/\.(png|jpg|jpeg|gif|webp|svg|woff2?|ttf|css|ico)(\?|$)/i.test(url))
  && (type === 'XHR' || type === 'Fetch' || type === 'Document' || type === 'Other' || !type);

export async function record(port, { seconds = 1800, outFile, log, signal, maxBytes, maxFiles } = {}) {
  const L = log || { ok() {}, info() {} };
  const writer = createRotatingLineWriter(outFile, { ...DEFAULT_TELEMETRY_ROTATION, maxBytes, maxFiles });
  const write = o => writer.writeJson(o);
...
export function statsFromFile(jsonlPath) {
  ...
  const parsed = forEachJsonlObject(jsonlPath, (e) => {
    if (e.kind === 'req') {
  ...
  return { events: parsed.records, bad: parsed.bad, hours: byHour, endpointAvgMs };
}
...
const forEachSubmission = (p, cb) => existsSync(p) ? forEachJsonlObject(p, cb) : { records: 0, bad: 0 };
...
export function resolveFromPlatform(ledgerPath, platform, ts) {
  ...
  const parsed = forEachSubmission(p, (e) => { if (e.k === 's') submitted.set(e.mid, e); else if (e.k === 'r') resolved.add(e.mid); });
...
export function passRateStats(ledgerPath) {
  ...
  forEachSubmission(submissionsPath(ledgerPath), (e) => { if (e.k === 's') submits.set(e.mid, e); else if (e.k === 'r') resolves.set(e.mid, e); });
  const byHour = {};
...
export function passRateArms(ledgerPath) {
  const submits = new Map(); const resolves = [];
  forEachSubmission(submissionsPath(ledgerPath), (e) => { if (e.k === 's') submits.set(e.mid, e); else if (e.k === 'r') resolves.push(e); });
  const arms = {};
  for (const e of resolves) { const s = submits.get(e.mid); if (!s) continue; update(arms, String(new Date(s.ts).getHours()), e.res === 1); }
