...
// jie3(drawer)：reopenDrawer → 1 次 UI 删抓 set-opt 签名 → 批量重放(UseLegoMid)。
// jie6(creative-tab)：搜索目标素材 → 1 次 UI 删抓 set-opt 签名 → 批量重放(UseLegoMid)。
// ★默认 dry-run；--apply 才真删。破坏性动作前必须 guardEnter，防止错账号/错计划。
import { readdirSync, statSync } from 'node:fs';
import { join } from 'node:path';
import { connect, norm, hardenViewport } from './cdp.mjs';
import { captureListSigs, injectNameFilter, pullRows } from './sync.mjs';
import { ensureVideo } from './state.mjs';
...
export function replayOk(repStr) {
  ...
  try {
    ...
    if (j && typeof j.code === 'number' && j.code !== 0) return false;
    if (j && typeof j.status_code === 'number' && j.status_code !== 0) return false;
  } catch (e) {}
  return true;
}

export function computeTargets(platform, localKeys, state, channel, kw) {
  const targets = [];
  const seen = new Set();
  for (const p of platform) {
    ...
    if (p.audit === 1 || p.audit === 4) targets.push({ name: p.name, legoMid: p.id, audit: 1 });
...
// 在 jie3 抽屉的"视频名称/ID"搜索框输入 term + 回车（渲染目标行用）。找不到框返回 false。
async function searchDrawerVideo(cdp, term) {
  ...
  await sleep(200); await cdp.send('Input.insertText', { text: String(term) });
...
async function reopenDrawerJie3(cdp, planId, kw) {
  ...
  await searchDrawerVideo(cdp, kw); // 搜 KW 渲染目标行
}
...
async function uiDeleteCapture(cdp, targetIds) {
  ...
  const findAndClickDelete = () => cdp.ev(`(function(){function vis(el){const r=el.getBoundingClientRect();return r.width>0&&getComputedStyle(el).display!=='none';}
    ...
    return null;})()`);
  let dc = await findAndClickDelete();
  // ★单目标 E_SELECTOR 修复：搜 kw 后目标行可能未渲染（尤其恰 1 个目标 / 分页在后）→ 用 legoMid 精确搜再试一次。
  if (!dc && targetIds.length) {
    await searchDrawerVideo(cdp, String(targetIds[0]));
    dc = await findAndClickDelete();
  }
  if (!dc) { off(); throw Object.assign(new Error('无可见目标行可删（列表未渲染目标行；已试 kw + legoMid 搜）'), { code: 'E_SELECTOR' }); }
...
async function clickBox(cdp, raw) {
  if (!raw) return false;
...
async function searchCreativeMaterial(cdp, id) {
  await cdp.send('Page.bringToFront');
  await closeOverlays(cdp);
  await cdp.j(`const tab=__byText(/^视频$/,'span,div,button,a,[role=tab]').pop(); if(tab)__synthClick(tab); return 1;`);
  await sleep(1000);
  const ok = await cdp.j(`const inp=[...document.querySelectorAll('input')].filter(__vis).find(i=>/(视频名称|ID)/.test(i.getAttribute('placeholder')||'')); if(!inp)return false; __setInput(inp,${JSON.stringify(id)}); inp.focus(); inp.dispatchEvent(new KeyboardEvent('keydown',{key:'Enter',code:'Enter',keyCode:13,which:13,bubbles:true})); inp.dispatchEvent(new KeyboardEvent('keyup',{key:'Enter',code:'Enter',keyCode:13,which:13,bubbles:true})); return true;`);
  if (!ok) throw Object.assign(new Error('creative-tab 未找到视频名称/ID搜索框'), { code: 'E_SELECTOR' });
  await sleep(5000);
}

async function uiDeleteCaptureCreative(cdp, targetIds) {
  const reqs = new Map();
  const off = cdp.onEvent(m => { if (m.method === 'Network.requestWillBeSent' && /material\/set-opt/.test(m.params.request.url)) reqs.set(m.params.requestId, { url: m.params.request.url, postData: m.params.request.postData || null }); });
  await cdp.send('Network.enable');
  let deletedId = null;
  try {
    for (const id of targetIds) {
      await searchCreativeMaterial(cdp, id);
      const more = await cdp.j(`const id=${JSON.stringify(id)}; const row=[...document.querySelectorAll('.ovui-tr,tr')].filter(__vis).find(r=>(r.innerText||'').includes(id)); if(!row)return null; const more=[...row.querySelectorAll('.oc-promotion-operation-more-action,[class*=more]')].filter(__vis)[0]; if(!more)return null; const r=more.getBoundingClientRect(); return JSON.stringify({x:Math.round(r.x+r.width/2),y:Math.round(r.y+r.height/2)});`);
      if (!more) continue;
      await clickBox(cdp, more);
      await sleep(900);
      const del = await cdp.j(`const items=[...document.querySelectorAll('button,span,div,a,[role=menuitem]')].filter(e=>__vis(e)&&(e.innerText||'').trim()==='删除'); const el=items[items.length-1]; if(!el)return false; __synthClick(el); return true;`);
      if (!del) continue;
      deletedId = id;
      break;
    }
    if (!deletedId) throw Object.assign(new Error('creative-tab 无可见目标行可删（更多菜单/删除入口未找到）'), { code: 'E_SELECTOR' });

    let confirm = null;
    for (let i = 0; i < 20 && !confirm; i++) {
      confirm = await cdp.j(`const btn=[...document.querySelectorAll('button')].filter(__vis).find(e=>(e.innerText||'').trim()==='确定'); if(!btn)return null; const r=btn.getBoundingClientRect(); return JSON.stringify({x:Math.round(r.x+r.width/2),y:Math.round(r.y+r.height/2)});`);
      if (!confirm) await sleep(300);
    }
    if (!confirm) throw Object.assign(new Error('creative-tab 删除确认弹窗未找到“确定”按钮'), { code: 'E_SELECTOR' });
    await clickBox(cdp, confirm);
    await sleep(3800);
  } finally {
    off();
  }
  let cap = null; for (const [, r] of reqs) if (r.url.includes('set-opt')) cap = r;
  return { sig: cap, deletedId };
}

export async function runDelete(cfg, { apply = false, log, platform = null, ledger } = {}) {
  const L = log || { step() {}, ok() {}, info() {}, warn() {} };
  const { system, target } = cfg;
  const channel = target.id;
  const { kw, flatRoot, ledgerPath } = system.project;
  if (target.ui !== 'drawer' && target.ui !== 'creative-tab') throw Object.assign(new Error(`不支持的 delete UI：${target.ui}`), { code: 'E_CONFIG' });

  // ★最小解耦最大合并（C 档）：编排器(prep/cycle/test-round)可把 sync 刚拉的 platform 快照穿进来 → 跳过这里重复的 reload+翻页拉取
  //   （sync 拉取前已 guardEnter 过同一标签）。dry-run + 穿入快照时连 CDP 都不连，纯本地算候选、瞬时返回。
  ...
  try {
    if (!platform) {
      ...
      await hardenViewport(cdp, system.chrome.viewport); // ★视口硬化：reopenDrawerJie3 按坐标点击计划/视频搜索框，后台/窄窗会点屏外（详见 cdp.mjs）
      await guardEnter(cdp, cfg, { log: L }); // ★A3: 入口护栏（断言 aavid/计划/会话 + 自愈一次），绝不在错标签/漂移态上删
    ...
    const targets = await led.read((state) => computeTargets(platform, localSet(flatRoot, kw), state, channel, kw));
    ...
    if (!cdp) {
      cdp = await connect({ port: target.port, aavid: target.aavid });
      await cdp.send('Runtime.enable');
      await hardenViewport(cdp, system.chrome.viewport); // ★视口硬化（同上：apply 路径走穿入快照、首次在此连接，坐标点击前先钉死视口）
      await guardEnter(cdp, cfg, { log: L });
    }
    let sig, deletedId;
    if (target.ui === 'drawer') {
      await reopenDrawerJie3(cdp, target.planId, kw);
      ({ sig, deletedId } = await uiDeleteCapture(cdp, targets.map(t => t.legoMid)));
    } else {
      ({ sig, deletedId } = await uiDeleteCaptureCreative(cdp, targets.map(t => t.legoMid)));
    }
    ...
    const repStr = rest.length
      ...
      : JSON.stringify({ status: 200, body: '{"status_code":0,"message":"skip-empty-rest"}' });
    ...
    await led.commit('delete:' + channel, (state) => {
