...
export async function runSync(cfg, { mutate = true, log } = {}) {
  ...
  if (mutate) { const nres = resolveFromPlatform(ledgerPath, platform, now); if (nres) L.info(`pass-rate: 结审 ${nres} 件`); }
...
export async function runSyncIncremental(cfg, { mutate = true, log, cacheSig = true } = {}) {
  ...
  // 连标签 → 入口护栏（断言账户/计划/会话，防错账户查询）→ 取 op 签名（★S1b 缓存优先）→ 按 mid 查审核
  const cdp = await connect({ port: target.port, aavid: target.aavid });
  ...
  try {
    ...
    if (!op) {
      ...
      const sigs = await captureListSigs(cdp);
  ...
  for (const r of rows) {
    ...
    if (r.audit != null) observed++; // 只计真学到审核值的（audit==null 时 observe 不改 last_status）
  }
  recomputeAll(state);
  if (mutate) saveState(ledgerPath, state);
