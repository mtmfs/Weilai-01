...
export async function runUpload(cfg, { names, log, mutate = true } = {}) {
  ...
  try {
    ...
    // ★Playwright 式视口硬化（根治 NO_CHOOSER 的环境层）：钉死虚拟视口 1920×1080 + 模拟聚焦 →
    //   页面布局与真实窗口大小/位置/前台与否彻底解耦、坐标恒定。窗口被拖小/后台/换显示器都不再让上传框跑屏外。
    try { await cdp.send('Emulation.setDeviceMetricsOverride', { width: 1920, height: 1080, deviceScaleFactor: 1, mobile: false }); } catch (e) {}
    try { await cdp.send('Emulation.setFocusEmulationEnabled', { enabled: true }); } catch (e) {}
