...
// ⚠️ v1 未加轮次 token 幂等：upload 重跑会双计 uploads（与 sync/delete 的"重跑续跑"不同）；轮间请先 sync、勿盲目重跑 upload。
export async function runUpload(cfg, { names, log, mutate = true } = {}) {
  ...
  if (mutate && injected.length) {
    const state = loadState(system.project.ledgerPath);
    const chs = loadChannels(); state.channels = chs.channels; state.pipeline = chs.pipeline;
    const now = Date.now();
    const injNames = injected.map(f => f.split(/[\\/]/).pop());
    for (const n of injNames) bumpUpload(ensureVideo(state, norm(n), n), channel, { ts: now });
    recomputeAll(state);
    saveState(system.project.ledgerPath, state);
    L.ok(`upload ${channel}: 注入 ${injNames.length} / 平台确认 ${N} → bump ${injNames.length} 件（uploads++、待 sync 回填 last_mid）`);
  }
