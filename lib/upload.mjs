...
export async function runUpload(cfg, { names, log, mutate = true, ledger, md5dir } = {}) {
  ...
  try {
    ...
    await hardenViewport(cdp); // ★视口硬化（根治 NO_CHOOSER 环境层）：钉死虚拟视口 1920×1080 + 模拟聚焦，坐标恒定（详见 cdp.mjs）
    ...
    if (injected.length) {
      ...
      try {
        const { rq, op } = await captureListSigs(cdp, { ui: target.ui });       // ★J1(复用J2)：creative-tab 补点素材触发列表
        if (target.ui === 'creative-tab') injectNameFilter(rq, kw);             // jie6 素材海量→只拉 kw 件，且找到刚传的 max-mid
