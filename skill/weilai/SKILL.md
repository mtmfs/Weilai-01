...
> **开工前**：先读 `I:\weilai-01\docs\AI-CONTEXT.md` 快速上下文；需要碰真实环境时再跑 `doctor` 体检。只有做架构变更、历史排障或已知 bug 判断时，才读 `I:\weilai-01\docs\工程总报告.md`。一切以仓库实况为准。
...
| `clean-artifacts [--apply]` | — | 清 telemetry/test/log 运行产物 | ⚠️ `--apply` 才删 |
...
2. **破坏性先 dry-run + 问**：delete/clear-local/clean-artifacts 的 `--apply`、任何 paid 写操作，先看清单、先确认。
