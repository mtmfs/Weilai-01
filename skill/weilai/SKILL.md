...
3. **paid = 真实花钱通道**：主管级、默认锁定。任何 paid 动作先确认、优先只读；**绝不擅自设 `WEILAI_SUPERVISOR=1` 跑 paid**，必须用户明确要求。
...
- **付费投放**（谨慎·主管级）→ 用户明确要求才 `WEILAI_SUPERVISOR=1` + 按场景用 `upload-paid` / `run-paid` / `run-both`。
